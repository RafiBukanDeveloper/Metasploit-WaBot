...
class PackageLoader {
  ...
  async load() {
    ...
    for (const filename of files) {
      ...
      try {
        if (ext === ".mjs") {
          this.Package[relativePath] = (await import(`file://${filename}`)).default;
        } else if (ext === ".js") {
          this.Package[relativePath] = require(filename);
        }
      } catch (e) {
        console.log(chalk.redBright(`❌ Gagal memuat [ ${relativePath} ]: `) + e);
        delete this.Package[relativePath];
      }
    }
  }
    ...
    watcher
      .on("add", async (filename) => {
        ...
        try {
          ...
          console.log(chalk.cyanBright(`📥 Package baru terdeteksi: ${filename}`));
          await this.load();
        } catch (e) {
          console.log(chalk.redBright(`❌ Gagal memuat [ ${relativePath} ]: `) + e);
        }
